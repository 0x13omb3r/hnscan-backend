...
class Hnscan extends EventEmitter {
  ...
  async getBlock(height, details = true) {
    ...
    return {
      ...
      nonce: entry.nonce,
      // Revival aliases: the 2020-06 frontend reads snake_case names.
      ...
      previous_hash: !entry.prevBlock.equals(consensus.ZERO_HASH)
        ? entry.prevBlock.toString("hex")
        : null,
      next_hash: next ? next.toString("hex") : null,
      mined_by: miner,
      num_tx: block.txs.length,
      extra_nonce: entry.extraNonce ? entry.extraNonce.toString("hex") : null
    };
  }
  ...
  async getTransaction(hash) {
    ...
    tx.inputs = inputs;
    tx.outputs = outputs;
    // Revival alias: the 2020-06 frontend reads tx.tx_id.
    tx.tx_id = tx.hash;
